...
export class JobsService {
  ...
  getOrganincCloudKeywords() {
    const url = this.config.baseApiUrl + 'organickwdsperformance/brandwide';
  ...
  getOrganicKeywordSummary(jobId, runId){
    const url = this.config.organicKeywordUrl + '/' + jobId + '/runHistory/' + runId + '/resultsummary';
    return this.http.authenticatedGet(url).toPromise().then(res => {
      return res;
    }).catch(err => {
      this.handleError(err);
